...
const departments: Department[] = [
  ...
  { id: 'ant', name: 'Antioquia', countryId: 'co' },
]
const cities: City[] = [
...
export async function getDepartments(countryId: string): Promise<Department[]> {
  return env.useMockApi
    ? departments.filter((item) => item.countryId === countryId)
    : http(`/departments/${countryId}`)
}
export async function getCities(departmentId: string): Promise<City[]> {
  return env.useMockApi
    ? cities.filter((item) => item.departmentId === departmentId && item.active)
    : http(`/cities/${departmentId}`)
}
